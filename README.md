...
- The Thermal / Fan section provides a read-only **Run Thermal / Fan Check** button that calls `scripts/thermal_fan_report.sh`. The report uses safe built-in macOS status commands such as `pmset -g therm` when available, attempts to identify thermal constraints from read-only status output, and prints clear unavailable warnings when CPU temperature or fan RPM cannot be obtained without privileged, SMC, or third-party access. It does not use sudo, control fans, write SMC values, load kexts, modify SIP, change kernel behavior, or alter system settings.
...
- Advanced options are limited to configuring whether the existing Spotlight pause, Time Machine pause, plugged-in system sleep prevention, and display sleep prevention steps run, plus manual read-only App Priority, Memory / Storage, and Thermal / Fan reporting. App Priority mutation is intentionally deferred until a safer privileged-helper authorization and restore model exists. Most other controls are disabled placeholders, and no additional system-changing behavior has been added. Thermal / Fan support is read-only monitoring; CPU temperature and fan RPM may be unavailable with built-in non-privileged methods. There is no fan control, cache cleaning, browser cleanup, automatic cleanup, launch daemon control, privileged helper, SIP modification, undervolting, MSR access, kext loading, or experimental CPU feature control.
