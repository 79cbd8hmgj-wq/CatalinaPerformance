...
Restoration is handled by `scripts/app_priority_restore.sh`, which reads only CatalinaPerformance's saved app-priority state, verifies process identity before restoring (owner, command, full command when available, and process start time when available), skips stale or closed processes, and prints a summary of attempted, successful, skipped, and failed restores. `scripts/performance_off.sh` also attempts this restore automatically when app-priority state exists, but continues with the existing pmset, Time Machine, and Spotlight restore flow if app-priority restore reports a problem.
