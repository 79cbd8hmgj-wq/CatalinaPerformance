...
grep -F 'experimental focused policy at nice -1' "$PANEL" >/dev/null || fail "focused Firefox warning missing"
grep -F 'Other known browsers use a conservative main-process-only policy at nice -2' "$PANEL" >/dev/null || fail "other-browser priority warning missing"
grep -F 'verified process family at nice -5' "$PANEL" >/dev/null || fail "sustained workload policy copy missing"
grep -F 'Firefox focused policy' "$PANEL" >/dev/null || fail "focused Firefox policy heading missing"
grep -F 'Parent/UI + GPU helper + one activity-selected content process' "$PANEL" >/dev/null || fail "focused Firefox target copy missing"
grep -F 'Maximum boosted: 3 processes at nice -1' "$PANEL" >/dev/null || fail "focused Firefox cap copy missing"
grep -F 'Experimental: compare against App Priority OFF' "$PANEL" >/dev/null || fail "focused Firefox experimental warning missing"
grep -F 'Tracked Firefox processes:' "$PANEL" >/dev/null || fail "tracked Firefox status missing"
grep -F 'Processes actually boosted:' "$PANEL" >/dev/null || fail "actual boosted status missing"
grep -F 'AppPriorityApplicationCanonicalizer()' "$PANEL" >/dev/null || fail "canonical application identity is not used"
! grep -F 'runningApplication.executableURL' "$PANEL" >/dev/null || fail "running helper executable must not become saved identity"
! grep -F 'browser-exclusion' "$PANEL" >/dev/null || fail "browser exclusion must not be introduced"
! grep -F 'Firefox is not supported' "$PANEL" >/dev/null || fail "Firefox must remain selectable"
grep -F 'guard selection.enabled else' "$PANEL" >/dev/null || fail "disabled selection must override stale priority status"
...
grep -F 'stable Firefox uses a focused nice -1 policy' "$MAIN" >/dev/null || fail "Performance Mode confirmation does not explain focused Firefox policy"
