...
SH2
chmod +x "$TMP"/*.sh "$TMP/fake-agent" "$TMP/lib"/*.sh
...
VALIDATE_STATUS=10 BACKGROUND_APPLY_STATUS=0 CORE_ON_STATUS=0 run "$TMP/performance_on_with_priority.sh" --yes
assert_log 'priority:validate
background:apply
core:on' 'ON ordering'

VALIDATE_STATUS=10 BACKGROUND_APPLY_STATUS=20 CORE_ON_STATUS=0 run "$TMP/performance_on_with_priority.sh" --yes
assert_log 'priority:validate
background:apply
core:on' 'verified rollback degrades without blocking core ON'

set +e
VALIDATE_STATUS=10 BACKGROUND_APPLY_STATUS=21 CORE_ON_STATUS=0 run "$TMP/performance_on_with_priority.sh" --yes >/dev/null 2>&1
status=$?
set -e
...
set +e
VALIDATE_STATUS=10 BACKGROUND_APPLY_STATUS=0 CORE_ON_STATUS=1 BACKGROUND_RESTORE_STATUS=0 run "$TMP/performance_on_with_priority.sh" --yes >/dev/null 2>&1
status=$?
set -e
...
RESTORE_STATUS=11 BACKGROUND_RESTORE_STATUS=0 CORE_OFF_STATUS=0 run "$TMP/performance_off_with_priority.sh"
assert_log 'priority:stop-and-restore
background:restore
core:off' 'OFF restores priority and settings before core completion'

RESTORE_STATUS=11 BACKGROUND_RESTORE_STATUS=0 CORE_EMERGENCY_STATUS=0 run "$TMP/emergency_restore_with_priority.sh" --yes
assert_log 'priority:stop-and-restore
background:restore
core:emergency' 'Emergency restore includes background settings'
...
for file in "$ROOT/scripts/performance_on_with_priority.sh" "$ROOT/scripts/performance_off_with_priority.sh" "$ROOT/scripts/emergency_restore_with_priority.sh" "$ROOT/scripts/lib/background_service_wrapper_common.sh"; do /bin/sh -n "$file" || fail "syntax: $file"; done
